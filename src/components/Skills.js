...
const Skill = ({ name, x, y }) => {
  return (
    <motion.div
      ...
      lg:py-2 lg:px-4 md:text-sm md:py-1.5 md:px-3 xs:bg-transparent xs:font-bold xs:text-dark
      "
...
const Skills = () => {
  return (
    <>
      ...
      <div
        className="w-full h-screen relative flex items-center justify-center rounded-full bg-circularLight lg:h-[80vh] sm:h-[70vh] xs:h-[60vh]
      
      lg:bg-circularLightLg  md:bg-circularLightMd  sm:bg-circularLightSm"
      >
        <motion.div
          className="flex items-center justify-center cursor-pointer rounded-full font-semibold bg-dark text-light p-8 shadow-dark lg:p-6 md:p-4 xs:text-xs xs:p-2"
          whileHover={{ scale: 1.05 }}
        >
          Web
        </motion.div>
